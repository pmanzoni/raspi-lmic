...
{
  _error = bcm2835_i2c_write((const char *) values, 1);
	if (_error == BCM2835_I2C_REASON_OK )
	{
		_error = bcm2835_i2c_read( (char*) values, length);
		if (_error == BCM2835_I2C_REASON_OK ) {
			return(1);
		}
	}
...
{
	_error = bcm2835_i2c_write( (const char*)values, length);
	if (_error == BCM2835_I2C_REASON_OK ) {
		return(1);
	}
	return(0);
